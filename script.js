...
const setupLightbox = () => {
  ...
  const updateImageTransform = () => {
    clampPan();
    const flip = image.dataset.flip === 'true' ? -1 : 1;
    image.style.transform = `translate(${panX}px, ${panY}px) scale(${flip * zoomLevel}, ${zoomLevel})`;
  ...
  const openLightbox = (index) => {
    ...
    currentIndex = index;
    image.dataset.flip = trigger.dataset.flip === 'true' ? 'true' : 'false';
    resetZoom();
    image.src = fullSrc;
